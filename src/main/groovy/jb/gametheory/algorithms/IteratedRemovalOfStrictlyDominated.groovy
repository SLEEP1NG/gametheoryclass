 ...
 *
 */
class IteratedRemovalOfStrictlyDominated {

	Object[] findPossibilitiesForNash(Object[][] matrix) {
		int numRowsBefore = matrix.length
		int numColumnsBefore = matrix[0].length
	...
	int rowIndexAlwaysLower(Object[][] matrix, int index, int comparison) {
		...
		for (columnNum in 0..matrix[0].length-1) {
			if (matrix[index][columnNum][0] >= matrix[comparison][columnNum][0]) {
				alwaysLower = false
			}
		}
		if (alwaysLower) {
